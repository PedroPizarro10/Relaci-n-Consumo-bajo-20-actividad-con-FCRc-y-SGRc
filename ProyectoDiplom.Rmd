...
```{r, BDatos_mod}
BDatos_mod <- BDatos %>% mutate(Consumo_20=`Consumo_ bajo_ 20` *100 , FCRc_porc=FCRc*100, SGRc_porc=SGRc*100) # %>% select(Year_class, Centro,Jaula, Consumo_20,FCRc_porc,SGRc_porc)
summary (BDatos_mod)
str(BDatos_mod)
```
...
```{r anova}
Modelo <- lmer(FCRc_porc ~ Consumo_20 + (1 | Centro), data = BDatos_mod)
```
...
#dwtest(FCRc_porc ~ Consumo_20 + (1 | Centro), data = BDatos_mod, alternative = c("two.sided"), iterations = 15)
...
# leveneTest(FCRc_porc ~ Consumo_20 + (1 | Centro), data = BDatos_mod, center = "median")

# bartlett.test(FCRc_porc ~ Consumo_20 + (1 | Centro), data = BDatos_mod)
